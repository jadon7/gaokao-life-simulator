  ...
  "acts": [
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "mainTrack": "life",
          "phase": "开局上桌",
          "comedyDevice": "领导一句自由发挥",
          "riasecAxis": [
            "R",
            "I"
          ],
          "conflict": "展示材料临时失踪，群里全在发问号。你要决定先按流程救场，还是先查清到底谁动了文件。",
    ...
    },
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "一次失误被群聊截图放大，你再硬扛就要成表情包。你要决定自己修，还是拉人一起收拾。",
    ...
    },
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "你投出去的东西没人回，只有学长指出描述太空。你要决定重写证据，还是继续用数量撞门。",
...
- 首次出现角色按 stateHints.castIntroRule 带身份前缀。
- 每张卡使用对应 outlineCard；A/B 对应 riasecAxis[0]/[1]，不要输出 riasec。
- scene.body 60-110 字；选项短；consequence 写选择后的直接后果。
...
function phasePromptForYear(year = 1) {
  const currentYear = Number(year || 1);
  if (currentYear <= 3) {
    return "大学早期：专业首锚、角色首登、第一次被看见。";
  }
  if (currentYear <= 6) {
    return "毕业分岔：考研/保研/实习/项目开始分流。";
  }
  if (currentYear <= 9) {
    return "初入现实：读研、实习、校招或第一份工作落地。";
  }
  if (currentYear <= 12) {
    return "城市平台：换城市、换平台、晋升、确定关系或分手。";
  }
  if (currentYear <= 15) {
    return "成年压力：收入、家庭、长期伴侣、职级和城市。";
  }
  return "收束落点：前辈身份、职业定位和亲密关系收束。";
}

function withPhasePrompt(template, year) {
  return template.replaceAll("{{PHASE_PROMPT}}", phasePromptForYear(year));
}

export function annualTaskPromptForYear(year) {
  return withPhasePrompt(vNextAnnualTaskPrompt, year);
}

function phasePromptForRange(startYear = 1, count = 1) {
  const total = Math.max(1, Math.min(Number(count || 1), 5));
  if (total === 1) return phasePromptForYear(startYear);
  return Array.from({ length: total }, (_, index) => {
    const year = Number(startYear || 1) + index;
    return `第${year}年：${phasePromptForYear(year)}`;
  }).join("；");
}

export function batchTaskPromptForStartYear(startYear, count = 1) {
  return vNextBatchTaskPrompt.replaceAll("{{PHASE_PROMPT}}", phasePromptForRange(startYear, count));
}
...
const defaultStoryCast = {
  relationName: "沈晚晴",
  relationGender: "女生",
  relationIntro: "总坐靠窗位、笔记写得像攻略的同班女生沈晚晴",
  roommateName: "周越",
  roommateIntro: "总在上课路上边走边吃早餐的吃货舍友周越",
  mentorName: "林老师",
  mentorIntro: "辅导员兼专业导师林老师，常在班群里用一句话把人点上台",
  externalName: "陈总",
  externalIntro: "合作方负责人陈总，说话不多但每次都带来新截止时间",
  familyName: "妈妈和二姨",
...
function relationshipArcHint(history = [], year = 1) {
  ...
  if (year <= 9) return cold > good ? "关系阶段：分手或复合苗头" : "关系阶段：确定关系或异地磨合";
  if (year <= 12) return cold > good ? "关系阶段：新恋情或复合窗口" : "关系阶段：承诺、见朋友或同居压力";
...
function castIntroHint(storyCast = defaultStoryCast, history = []) {
  ...
  const intros = [
    ...
    .map(([, intro]) => intro);
  return intros.length ? `首次出现必须带身份短语：${intros.slice(0, 3).join("；")}` : "";
}
...
export function buildAnnualInput({ profile, history, year, totalGameYears = 18 }) {
  ...
  if (educationState) stateHints.educationState = educationState;
  return {
