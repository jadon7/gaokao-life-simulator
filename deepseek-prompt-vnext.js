...
- 只有固定伴侣有姓名；没有固定姓名时只写关系称呼；毕业后不生成新姓名。
...
function castIntroPromptForRange(startYear = 1, count = 1) {
  return "- 有 stateHints.castIntroRule 时，scene/relationshipTrack 首次写伴侣用完整称呼。";
}
...
const relationTraitPool = [
  { trait: "清冷学霸", intro: name => `同班学霸${name}` },
  { trait: "天然呆", intro: name => `隔壁班同学${name}` },
  { trait: "温柔白月光", intro: name => `项目搭档${name}` },
  { trait: "校花", intro: name => `社团同学${name}` }
];
...
function buildStoryCast(profile = {}) {
  const rawRelationName = String(profile.relationName || "").trim();
  const relationName = [...castNamePools.female, ...castNamePools.male].includes(rawRelationName) ? rawRelationName : "";
...
function directorText(value, storyCast = defaultStoryCast) {
  const relationIntro = storyCast?.relationIntro || storyCast?.relationName || defaultStoryCast.relationIntro;
...
function relationRedactionTerms(storyCast = defaultStoryCast) {
  return [
...
function castIntroHint(storyCast = defaultStoryCast, history = []) {
  ...
  const appeared = new Set(appearedRoleNames(history));
  const recentIntro = relationIntroFromRecentStory(storyCast, history);
  const intros = [
    [storyCast.relationName, recentIntro || storyCast.relationIntro],
  ]
    .filter(([, intro]) => intro && !appeared.has(intro) && !text.includes(intro))
    .map(([, intro]) => intro);
  return intros.length ? `伴侣首次出场：${intros.slice(0, 3).join("；")}` : "";
}
...
export function buildResultInput({ profile, history, totalGameYears = 18, finalResultAge = 35 }) {
  return {
    ...
    yearningStats: buildYearningStats(history),
    resultHints: {
