    ...
    },
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "最后一张牌不是问你成没成功，而是问你愿意把哪句真话留给十八岁的自己。你要讲体面路线，还是讲真实代价。",
...
function majorAnchorHint(profile = {}, year = 1, history = []) {
  ...
  const anchor = pool[(hashString(`${text}:${year}`) || 0) % pool.length];
...
function recentIncidentsHint(profile = {}, history = []) {
...
export function buildAnnualInput({ profile, history, year, totalGameYears = 18 }) {
  ...
  const stateHints = {
    ...
    majorAnchor: majorAnchorHint(profile, year, history),
  ...
  const recentIncidents = recentIncidentsHint(profile, history);
...
export function buildBatchInput({ profile, history, startYear, count, totalGameYears = 18 }) {
  ...
  const stateHints = {
    ...
    repeatGuard: repeatGuard(history),
    stageGuard: stageGuard(history),
    majorAnchor: majorAnchorHint(profile, startYear, history),
