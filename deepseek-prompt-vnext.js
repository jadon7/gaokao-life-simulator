  ...
  "acts": [
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "暑期实习名单出来，远方平台和本地机会第一次同时招手，你们也第一次要认真谈异地。你要把异地规则说清，还是先把城市窗口拿下。",
          ...
          "abType": "说清异地 / 拿下窗口",
          "summaryTask": "现实线同步推进，但不要把伴侣写成事业阻碍",
          "callbacks": [
            "暑期实习",
            "异地规则",
            "第一次去留"
    ...
    },
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "转正答辩结束，城市续约和同居计划摆到桌面上。你要一起拍板下一站，还是先把各自节奏讲清。",
          "sideBeat": "收入和城市落点开始影响长期关系",
          "characters": [
            "关系线核心角色",
            "朋友群像"
          ],
          "abType": "一起拍板 / 讲清节奏",
          "summaryTask": "现实线只写一个具体落点，不写伴侣妨碍事业",
          "callbacks": [
            "同居计划",
            "城市续约",
            "第一次拍板"
    ...
    },
    {
      ...
      "cards": [
        ...
        },
        {
          ...
          "conflict": "父母照护、孩子择校或房贷重排摆到桌面上，不能再拖。你要和伴侣拿出具体行动，还是先把现实排稳。",
...
- 有 stateHints.relationshipPressure 时，只作为一句家庭投入提醒，不改变本年主决策。
...
- 不写 recentSceneObjects 里的道具；伴侣/新恋情可用名字，其他角色用统称或关系称呼。
{{CAST_INTRO_PROMPT}}
- 每张卡使用对应 outlineCard；A/B 对应 riasecAxis[0]/[1]，不要输出 riasec。
- summary 写上一年余波；scene.body 写本年事故；选项用 label 写完整互斥动作；consequence 写选择后的直接后果。
...
function phasePromptForYear(year = 1, history = []) {
  ...
  const prompts = {
    ...
    8: educationState === "继续读研" ? "研究生毕业第一站：入职、读博或规培落点定下来。" : "关系定调：转正、城市落点和共同生活必须拍板。",
    9: "口碑危机：一次职业失误影响后续机会。",
...
function castIntroPromptForRange(startYear = 1, count = 1) {
  const from = Number(startYear || 1);
  const to = from + Math.max(1, Number(count || 1)) - 1;
  return from <= 5 && to >= 1
    ? "- 首次出现角色使用 stateHints.castIntroRule 里的身份短语。"
    : "";
}
...
function compactOutlineCardWithClosing(card, storyCast = defaultStoryCast, closingFrame = "") {
  ...
  return {
    ...compact,
    comedyDevice: "最后一句话",
    conflict: `${closingFrame}。最后一张牌只做回顾收束：把开局、一次关键代价和现在的落点讲清楚。`,
...
function supportRolesForYear(year = 1, history = []) {
  ...
  if (currentYear <= 15) return ["客户", "老板", "团队成员", "家里长辈", "孩子老师"];
  return ["下属", "合伙人", "客户", "老板", "团队成员"];
}

function familyRolesForYear(year = 1) {
  const currentYear = Number(year || 1);
  if (currentYear <= 6) return ["妈妈", "爸爸", "二姨"];
  if (currentYear <= 12) return ["妈妈", "爸爸", "二姨", "伴侣家人"];
  return ["妈妈", "爸爸", "二姨", "家里长辈", "孩子老师"];
}
...
function compactStoryCastForYear(profile = {}, existingCast = null, year = 1, history = [], relationshipStage = "", includeRelation = false) {
  ...
  const currentYear = Number(year || 1);
  const baseCast = {
    supportRoles: supportRolesForYear(currentYear, history),
    familyRoles: familyRolesForYear(currentYear),
    mentorIntro: cast.mentorIntro,
    familyIntro: cast.familyIntro
  };
  if (currentYear <= 5 || includeRelation) {
...
function majorAnchorHint(profile = {}, year = 1, history = []) {
  ...
  const nextIncident = currentIncidentHint(profile, currentYear + 1, history, getOutlineCard(currentYear + 1)).replace(/^本年事故：/, "");
...
function currentIncidentHint(profile = {}, year = 1, history = [], outlineCard = null) {
  ...
  if (currentYear === 2) return `本年事故：${items[0].label}`;
...
function relationshipPressureHint(history = [], year = 1) {
  ...
  const careCount = (recentText.match(/家庭|孩子|父母|老人|照护|买房|房贷|结婚|同居|见家长|异地|陪|沟通|一起/g) || []).length;
  if (careerCount < 4 || careCount > 1) return "";
  const stage = relationshipStageHint(history, year);
  if (["分手收束", "体面告别", "新恋情萌芽"].includes(stage)) {
    return "家庭投入提醒：家人觉得你最近只给工作留时间；一句带过。";
  }
  return "家庭投入提醒：伴侣或家人觉得你最近只给工作留时间；一句带过。";
}
...
function castIntroHint(storyCast = defaultStoryCast, history = []) {
  ...
  const intros = [
    ...
    .filter(([name, intro]) => name && intro && !appeared.has(name) && !text.includes(name))
    .map(([, intro]) => intro);
  return intros.length ? `可用身份短语：${intros.slice(0, 3).join("；")}` : "";
}
...
export function buildAnnualInput({ profile, history, year, totalGameYears = 18 }) {
  ...
  const compactCast = compactStoryCastForYear(profile, storyCast, year, history, relationshipStage, outlineCard?.mainTrack === "relationship" || /伴侣/.test(relationshipPressure));
...
export function buildBatchInput({ profile, history, startYear, count, totalGameYears = 18 }) {
  ...
  const compactCast = compactStoryCastForYear(profile, storyCast, startYear, history, relationshipStage, firstOutlineCard?.mainTrack === "relationship" || /伴侣/.test(relationshipPressure));
  const newRelation = newRelationHint(storyCast, relationshipStage);
  const recentCallbacks = getRecentCallbacks(history, 4)
    .filter(seed => educationState !== "已放弃考研" || !/考研|保研|复习|绩点/.test(seed));
  const stateHints = {
